        ...
        if entity_fov:
            for y in range(map.height):
                for x in range(map.width):
                    ...
                    if visible:
                        if wall:
                            tcod.console_set_char_background(self.console, x, y, COLORS.get('light_wall'),
                                                             tcod.BKGND_SET)
                        else:
                            tcod.console_set_char_background(self.console, x, y, COLORS.get('light_ground'),
                                                             tcod.BKGND_SET)
                        map.tiles[x][y].explored = True
                    elif map.tiles[x][y].explored:
                        if wall:
                            tcod.console_set_char_background(self.console, x, y, COLORS.get('dark_wall'),
                                                             tcod.BKGND_SET)
                        else:
                            tcod.console_set_char_background(self.console, x, y, COLORS.get('dark_ground'),
                                                             tcod.BKGND_SET)
